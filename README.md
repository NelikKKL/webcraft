...
**Статус: этап 3 — decomp-дерево (446 файлов) скопировано и полностью
пропатчено, WebMinecraft/WebEntryPoint переписаны на реальный запуск игры**
(ResourcePreloader → WebMinecraft.init() → requestAnimationFrame-цикл через
runOneFrame() — оригинальный блокирующий `run()`-with-`while` разбит на
`init()`+`runOneFrame()`, см. TODO.md). **Ничего из этого не подтверждено
сборкой** (в этой среде разработки по-прежнему нет компилятора) — есть
как минимум 3 известные категории риска для следующего CI-прогона
(`java.util.zip`, `java.net.Socket`, `java.io.File` — см. TODO.md, раздел
"НОВЫЕ РИСКИ"), плюс обычная вероятность неучтённых мелочей при таком
объёме кода.

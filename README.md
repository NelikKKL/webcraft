...
**Статус: этап 3 — decomp-дерево скопировано и пропатчено. Три реальных
CI-прогона пройдены итеративно: первые два вскрыли ошибки `javac`
(JPMS-конфликт, API TeaVM, отсутствующие классы), третий — уже прошёл
`javac` полностью чисто и упал ГЛУБЖЕ, внутри самого TeaVM на этапе
reachability-анализа** (методы вроде `System.exit`/`Runtime.maxMemory`,
которые реально существуют в JDK и потому проходят javac, но не
реализованы в собственной JRE-эмуляции TeaVM — качественно другой класс
ошибок, см. TODO.md). Все найденные проблемы исправлены, включая
исключение нерабочей (уже в оригинальной игре) фичи скачивания звука по
HTTP и создание `net.minecraft.client.netshim` для `Socket`/`ConnectException`
(тот же JPMS-конфликт с `java.base`, что и с AWT/`java.desktop` ранее).
**Ничего не подтверждено четвёртым прогоном** — остаются `java.util.zip`,
`java.io.File`, `org.lwjgl.BufferUtils` как непроверенные риски (см. TODO.md).

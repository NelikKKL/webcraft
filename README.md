...
**Статус: этап 3 — decomp-дерево скопировано и пропатчено. Игра успешно
собралась И ЗАПУСТИЛАСЬ в браузере пользователя** — первая веха такого
уровня. Поймана и исправлена первая runtime-ошибка (не ошибка сборки!):
TeaVM-баг кодогенерации превратил `f6 - -0.0f` в синтаксически битый JS
(`e--0.0`, парсится браузером как декремент+оторванный токен) — см.
TODO.md за полным разбором и универсальным способом искать такие ошибки
в будущем (проверено напрямую по скомпилированному `alpha126.js`, не
только по Java-исходнику — подтверждено единственное совпадение во всём
файле). Добавлена веб-обвязка по запросу: экран загрузки на
`background.png` с прогресс-баром, ассеты одним `web/assets.zip`
(JSZip через CDN), экран краша с логом и копированием — см. TODO.md.
Остаются `java.util.zip`, `java.io.File`, `org.lwjgl.BufferUtils` как
непроверенные риски для дальнейшей игровой логики (см. TODO.md).

...
**Статус: этап 3 — decomp-дерево скопировано и пропатчено. Четыре реальных
CI-прогона пройдены итеративно** — первые два вскрыли `javac`-ошибки,
третий прошёл `javac` чисто и упал внутри TeaVM (reachability-анализ:
`System.exit`/`Runtime.maxMemory`/`Socket`/`ConnectException` и др. —
методы, реально существующие в JDK, но не реализованные в собственной
JRE-эмуляции TeaVM), четвёртый — всего одна оставшаяся ошибка
(`InetAddress`, тот же `java.net`/`java.base` конфликт). **Ничего не
подтверждено пятым прогоном** — остаются `java.util.zip`, `java.io.File`,

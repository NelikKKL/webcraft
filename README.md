...
РЕНДЕРИТСЯ В БРАУЗЕРЕ** (подтверждено скриншотом: логотип главного меню
Minecraft, отрисованный через настоящий WebGL2-пайплайн). По пути найдены
и исправлены: TeaVM codegen bug (`oz.java`), отсутствие автозапуска
(`window.main()`), полный отказ от JSZip в пользу собственной WASM-
библиотеки `akrile`, и **критичный системный баг** — неверный вызов
`@JSFunctor`-callback'ов (`callback.methodName()` вместо `callback()`)
в 4 файлах, из-за которого ни одна текстура не грузилась, игровой цикл
не мог сделать больше одного кадра, а ввод не работал вообще (см.
TODO.md за полным разбором). **Открытый вопрос** — на отрендеренном
главном меню не видно кнопок и анимированного 3D-фона, не диагностировано
в рамках этой сессии (см. TODO.md). Остаются `java.util.zip`,
`java.io.File`, `org.lwjgl.BufferUtils` как непроверенные риски для
дальнейшей игровой логики.

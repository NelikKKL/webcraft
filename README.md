...
**Статус: этап 3 — decomp-дерево скопировано и пропатчено, два реальных
CI-прогона дали в сумме 48 ошибок компиляции — все проанализированы и
исправлены** (JPMS-конфликт, точный API `Uint8Array`/`Float32Array` в
TeaVM, отсутствующие `ARBOcclusionQuery`/`Sys`/`enableRepeatEvents`,
4 файла из мёртвого isom/звукового поддерева, не пойманные
предыдущим статическим анализом — см. TODO.md за полными деталями и
уроками каждого раунда). **Ничего из этого не подтверждено третьим
прогоном** (в этой среде разработки по-прежнему нет компилятора) — есть
как минимум 4 известные категории риска (`java.util.zip`, `java.net.Socket`,
`java.io.File`, `org.lwjgl.BufferUtils` — см. TODO.md).

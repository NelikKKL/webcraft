...
TeaVM). **Первая полностью успешная сборка получена.** Добавлена веб-обвязка
по запросу: экран загрузки на `background.png` с прогресс-баром, все
ассеты упакованы в один `web/assets.zip` (распаковка через JSZip прямо в
браузере вместо 44 отдельных HTTP-запросов), экран краша с полным логом
и кнопкой копирования — см. TODO.md. **Не подтверждено реальным запуском
в браузере** (в этой среде разработки нет браузера для проверки) —

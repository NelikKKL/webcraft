...
собралась и запустилась в браузере пользователя, поймана и исправлена
runtime-ошибка (TeaVM codegen bug, `oz.java`) и критичный баг отсутствия
автозапуска (`window.main()` никогда не вызывался).** По запросу
пользователя JSZip+CDN полностью заменён на собственную WASM-библиотеку
`akrile` (свой бинарный формат архива, без внешних CDN-зависимостей) —
проверено напрямую в headless Chromium (доступен в этой среде — см.
TODO.md): инициализация WASM, скачивание и распаковка `assets.akrile`,
декодирование PNG — всё подтверждено рабочим на реальных данных.
Остаются `java.util.zip`, `java.io.File`, `org.lwjgl.BufferUtils` как
непроверенные риски для дальнейшей игровой логики, и сама пересборка
`alpha126.js` с учётом akrile-изменений — не проверена (нет TeaVM в этой
среде), покажет следующий CI-прогон (см. TODO.md).

...
public class oz {
    ...
    public float[] b(float f2, float f3) {
        ...
        if (f6 >= -0.0f - f5 && f6 <= -0.0f + f5) {
            // ПАТЧЕНО для web-порта: `f6 - -0.0f` компилировался TeaVM в JS
            // как `...--0.0`, что браузер парсит как постфиксный декремент
            // (`x--`), за которым следует оторванный токен `0.0` — реальная
            // ошибка сборки: "SyntaxError: missing ) in parenthetical" в
            // рантайме. Вычитание -0.0f математически ничего не меняет
            // здесь (f6 сразу используется в делении/умножении дальше, где
            // разница между +0.0 и -0.0 не имеет значения) — убрано целиком
            // вместо просто "почистить форматирование", чтобы не оставлять
            // потенциально то же самое узкое место для будущих правок.
